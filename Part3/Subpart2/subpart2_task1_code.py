...
def Forward_kinematics(angle_1,angle_2):
	...
	'''
	a = cos(angle_2)
	c = tan(angle_1 - atan((l2*sin(angle_2))/(l1 + l2*a)))
	a = a*2*l1*l2 + l1**2 + l2**2
	y = sqrt(a/(1+c**2))
	z = sqrt(a*c*c/(1+c**2))
	# y=0 # calculate yatan((l2*sin(angle_2))/(l1 + l2*a)
	# z=0 # calculate z
	return [0,y,z]
...
def Inverse_kinematics(target):
	...
	c = atan(y/x)
	# print(a,b,c)
	angle_2 = -acos(a)#calculate angle_2
	b = atan((l2*sin(angle_2))/(l1 + l2*a))

	angle_1 = c + b  #calculate angle_1
	# angle_1 = 1.57 - angle_1
...
while(True):
	...
	if z>=1:
		z = 0.1
	angle = []
	angle1, angle2 = Inverse_kinematics([0,y,z])
	angle.append(angle1)
	angle.append(angle2)
	# if (i%100 == 0):
	# 	print(y, z)
